...
document.getElementById("signupForm").addEventListener("submit", async (e) => {
  ...
  try {
    // মোবাইল নম্বরের uniqueness নিশ্চিত হয় pseudoEmail-এর মাধ্যমে Firebase Auth নিজেই
    // (নিচে createUserWithEmailAndPassword duplicate হলে auth/email-already-in-use ছুড়বে)।
    // sign-in হওয়ার আগে mobileIndex collection read করার permission থাকে না (Firestore rules
    // অনুযায়ী শুধু signed-in ইউজাররাই এটা read করতে পারে), তাই এখানে আলাদা pre-check বাদ দেওয়া হলো।

    // ১. Firebase Auth অ্যাকাউন্ট তৈরি
    ...
    // ২. প্রোফাইল ছবি Cloudinary-তে আপলোড
    const photoURL = await uploadImageToCloudinary(selectedAvatarFile, `avatars/${uid}`);

    // ৩. Firestore-এ ইউজার প্রোফাইল ও মোবাইল ইনডেক্স সংরক্ষণ (batch — atomicity)
    // mobileIndex ডকুমেন্টে mobile ফিল্ডও রাখা হচ্ছে, কারণ rules-এ doc-id ও ফিল্ড মিল যাচাই করা হয়
    ...
    batch.set(db.collection("mobileIndex").doc(mobile), { uid, mobile });
    await batch.commit();
